/**
 * 케이스 스터디 데이터.
 * 프로젝트를 추가하려면 이 객체에 새 항목만 넣으면 /work/[slug]에서 자동 렌더된다.
 * (Recto는 전용 다이어그램이 많아 별도 컴포넌트로 유지: /work/recto-ab)
 *
  ...
  "loan-compare": {
    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          "신뢰를 더한 랜딩은 전환율을 6%p 끌어올렸고, 입력 흐름 개선은 전 연령대 가심사 전환율을 5.74%p 높였어요. 가장 인상적이었던 건 dead end였던 부결 페이지 — 클릭률 32%, 전환율 47.8%가 올랐어요.",
          "데이터가 ‘왜’를 말해주지 않았던 자리에 세운 가설들이, 결국 숫자로 증명된 셈이에요.",
        ],
      },
      {
        n: "08",
        kicker: "Takeaway",
        title: "데이터가 멈춘 곳을, 가설과 디자인으로 이었다.",
        body: [
          "이 프로젝트의 핵심은 데이터가 답을 주지 않을 때의 태도였어요. ‘왜’가 비어 있으면 가설을 세우고, 아무도 보지 않던 지표(부결자 40%)에서 기회를 찾고, dead end였던 화면을 ‘다음 선택지’로 바꿨어요.",
          "신뢰는 로고로, 이탈은 유관 상품으로, 입력 부담은 명확한 선택 상태로 — 데이터의 공백마다 디자인으로 가설을 검증할 자리를 만들었어요.",
        ],
        pull: "데이터가 말해주지 않는 곳에, 디자이너의 가설이 들어간다.",
      },
    ],
  },
};
